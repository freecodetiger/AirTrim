...
路线：**M0** ASR 验证（✅）→ **M1** 字幕工具（✅）→ **M2** 一键紧凑（✅）→ **M3** AI 建议闭环（✅）→ **M4** 项目管理 / AI 辅助（✅）。v0.1.2 已发布（DMG 暂为 ad-hoc 签名，公证待 owner 证书，见 [docs/release.md](docs/release.md)）；后续方向见 [docs/roadmap.md](docs/roadmap.md)。
